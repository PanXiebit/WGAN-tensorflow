...
def generator_conv(z):
    ...
    print(train)
    train = ly.conv2d_transpose(train, num_outputs=256, kernel_size=3, stride=2,
...
def critic_conv(img, reuse=False):
    with tf.variable_scope('critic') as scope:
        ...
        img = ly.conv2d(img, num_outputs=size, kernel_size=3,
                        stride=2, activation_fn=lrelu)  # [batch, 16, 16, 64]
        img = ly.conv2d(img, num_outputs=size * 2, kernel_size=3,
                        stride=2, activation_fn=lrelu, normalizer_fn=ly.batch_norm)  # [batch, 8, 8, 128]
        img = ly.conv2d(img, num_outputs=size * 4, kernel_size=3,
                        stride=2, activation_fn=lrelu, normalizer_fn=ly.batch_norm)  # [batch, 4, 4, 256]
        img = ly.conv2d(img, num_outputs=size * 8, kernel_size=3,
                        stride=2, activation_fn=lrelu, normalizer_fn=ly.batch_norm)  # [batch, 2, 2, 512]
        logit = ly.fully_connected(tf.reshape(
            img, [batch_size, -1]), 1, activation_fn=None)  # [batch, 1]
    return logit
...
def build_graph():
    ...
    return opt_g, opt_c, real_data, c_loss, g_loss, fake_logit, true_logit
...
def main():
    ...
    with tf.device(device):
        ...
        opt_g, opt_c, real_data, c_loss, g_loss, fake_logit, true_logit = build_graph()  # opt_g, opt_c, real_data = build_graph()
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(max_iter_step):
            ...
            for j in range(citers):
                ...
                if i % 100 == 99 and j == 0:
                    ...
                    _, merged, c_loss_np, fake_logit_np, true_logit_np = sess.run(
                        [opt_c, merged_all, c_loss, fake_logit, true_logit], feed_dict=feed_dict,
                    ...
                    tf.logging.info("Step = {}, c_loss = {:.4f}, f_logit = {}, r_logit = {}"
                                    .format(i, c_loss_np, fake_logit_np[:5], true_logit_np[:5]))
                else:
                    _, c_loss_np = sess.run([opt_c, c_loss], feed_dict=feed_dict)
                    #if i % 100 == 99:
            ...
            if i % 100 == 99:
                ...
                _, merged, g_loss_np, fake_logit_np = sess.run([opt_g, merged_all, g_loss, fake_logit], feed_dict=feed_dict,
                                                               options=run_options, run_metadata=run_metadata)
                summary_writer.add_summary(merged, i)
                summary_writer.add_run_metadata(
                    run_metadata, 'generator_metadata {}'.format(i), i)
                tf.logging.info("Step = {}, g_loss = {:.4f}, f_logit = {}".format(i, g_loss_np, fake_logit_np[:5]))
            else:
